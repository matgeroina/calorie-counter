...
export default function AddFood() {
  ...
  if (!Number.isFinite(kcal) || kcal <= 0) return alert("Enter calories greater than 0");
    ...
    return (
        <section className="card">
            ...
            <form onSubmit={onSubmit} className="form">
                ...
                <label className="label">
                    Food Name 
                    <input className="input" value={name} onChange={(e) =>setName(e.target.value)} placeholder="e.g., chicken wrap" />
                </label>
                <label className="label">
                    Calories 
                    <input className="input" type="number" min= "1" value={calories} onChange={(e) => setCalories(e.target.value)} placeholder="e.g., 250" />
                </label>
                <div className="card" style={{ marginTop: 0 }}>
                <div className="muted" style={{ marginBottom: 8 }}>Quick add</div>
                <div className="row">
                    {QUICK_PRESETS.map((p) => (
                      <button key={p.name} type="button" className="btn" onClick={() => addPreset(p)} >
                      {p.name} • {p.calories}
                    </button>
    ))}
  </div>
